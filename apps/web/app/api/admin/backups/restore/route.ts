...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${config.apiBaseUrl}/api/admin/backups/restore`, {
    method: "POST",
    headers: {
  ...
  ["/", "/rankings", "/search", "/compare", "/admin", "/admin/backups", "/admin/logs"].forEach((path) => revalidatePath(path));
